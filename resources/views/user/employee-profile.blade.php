    ...
    <div class="row">
      <div class="col-12">
        <div class="card" >
          ...
          <div id="divEditPDS" class="card-footer" hidden="true">
            <label>Upload New PDS</label>
            <input type="file" name="pds" class="form-control"/>
          </div>
          <div id="divPDS" class="card-footer" >
            @if($employee->pds_file!=NULL)
            <a href="{{ asset('uploads/pds/'.$employee->pds_file) }}" target="_blank"><i class="fa fa-download"> Download PDS</i></a>
            @else
            <label class="text-danger">*No PDS uploaded*</label>
            @endif
          </div>
        </div>
      </div>
        ...
        <div class="card-body ">
          <!-- Nav tabs -->
            <ul class="nav nav-tabs" id="myTab" role="tablist">
              ...
              <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#schedule" role="tab" aria-selected="false">Schedule</a>
              </li>
            </ul>
          <!-- Tab panes -->
            <div class="tab-content">
                    ...
                    <div class="col-sm-4">
                      <select disabled="true" class="form-control-plaintext fieldToEdit" name="profession" id="profession">
                        @foreach($professions as $profession)
                          <option value="{{ $profession->profession }}" @if($employee->profession==$profession->profession) selected @endif >{{ $profession->profession }}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
              ...
              <div class="tab-pane" id="schedule" role="tabpanel">
                <table id="tblPayroll" class="table" style="width:100%">
                  <thead>
                    <tr>
                        <th>Date</th>
                        <th>Time In</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{ Carbon\Carbon::parse($sched->sched_in)->format('H:i A') }}</td>
                      <td>{{ Carbon\Carbon::parse($sched->sched_out)->format('H:i A') }}</td>
                    </tr>
                    @endforeach
                  </tbody>
                </table>
...
<script type="text/javascript">
  ...
  $('#btnEditFields').on('click', function() {
    ...
    $('#btnEditFields').hide();
    $('#divEditPDS').prop('hidden',false);
    $('#divPDS').prop('hidden',true);
  });
  ...
  $('#btnCancelEdit').on('click', function() {
    ...
    $('#btnEditFields').show();
    $('#divEditPDS').prop('hidden',true);
    $('#divPDS').prop('hidden',false);
  });
